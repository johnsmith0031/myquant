...
class TGarch():
    ...
    def get_loss(self,pars,renew_params = False):
        ...
        status = np.zeros(hlen)
        status[ep<0] = 1
        if self.beta == 0 and self.omega == 0:
            op = uo
        else:
            for i in range(self.start_loc,hlen):
                op[i] = uo + op[(i-self.beta):i].dot(betas) +\
                        ep2[(i-self.omega):i].dot(omegas+status[(i-self.omega):i]*lnys)
...
if __name__ == '__main__':
    ...
    x = ret
    tgarch = TGarch(x,0,0,1,1)
    tgarch.train()
    garch = Garch(x,0,0,1,1)
#    sigma,p_values = tgarch.get_param_stats()
#    print(np.round(tgarch.pars,7))
#    print(np.round(p_values,2))
    plt.plot(tgarch.sigma)
    print(np.sum(np.abs(x)>tgarch.sigma*1.96)/len(x))
    rp,op = tgarch.get_next()
